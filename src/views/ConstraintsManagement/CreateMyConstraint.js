...
function CreateMyConstraint() {
    ...
    await axios
      ...
      .then((response) => {
        ...
        for(let i=0; i < data.length; i++) {
          if (moment() <= moment(data[i].startDate).subtract(2, 'weeks')) {
            chosenCsmind = data[i];
            break;
          }
        }

        if (chosenCsmind !== null) {
          setConstraint({
            ...constraint,
            csmindId: chosenCsmind.id
          })
          setSelectedCsmind(chosenCsmind);
          getDayOfWeeks(chosenCsmind);
        }
      })
  ...
  useEffect(() => {
    if(token['Token']) {
      ...
      let role = decoded.userInfo.roles.includes("Lecturer") ? "lecturer" : "student";
      setRole(role);
    }
  }, [token]);
              ...
              >
                {
                  csmindSelection && csmindSelection.map((item) => (
                    <MenuItem key={item.id} value={item.id} disabled={ moment() <= moment(item.startDate).subtract(2, 'weeks') && item.schedule === null  ? false : true }>
...
export default CreateMyConstraint;