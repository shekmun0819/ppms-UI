...
const HEADERS = ['#', 'MatricNo', 'Student', 'Title', 'Supervisor', 'Examiner', 'Chair', 'Venue', 'Time', 'Day', 'Date'];
...
function MasterSchedule() {
  ...
  const fetchCsminds = async () => {
    let url = roles.includes("Admin") ?  API_URL + "/api/v1/csmind/getAll" :  API_URL + "/api/v1/csmind/getUserCsminds/" + userId;
  ...
  const onExportPdf = () => {
    if (dayOfSlots.length > 0) {
      ...
      for (let i = 0; i < dayOfSlots.length; i++) {
        ...
        const input = document.getElementById(`masterSchedule_${i}`);
        html2canvas(input)
          .then((canvas) => {
            ...
            if(i === dayOfSlots.length-1) {
              if(dayOfSlots.length > 1) {
                doc.save(`Master Schedule ${dayDate[0]} - ${dayDate[dayOfSlots.length-1]}.pdf`);
              }
              else {
                doc.save(`Master Schedule ${dayDate[0]}.pdf`);
              }
            }
        });
      }
  ...
  const onExportExcel = () => {
    ...
    for (let i = 0; i < dayOfSlots.length; i++) {
      let worksheet = XLSX.utils.table_to_sheet(document.getElementById(`masterSchedule_${i}`), { origin: 3 });
      XLSX.utils.sheet_add_aoa(
        worksheet,
        [['CSMInD Date: ' + dayDate[i]], ['Academic Year, Semester : ' + `${selectedCsmind.academicSession.academicSession}, ${selectedCsmind.academicSession.semester}`]],
        { origin: 0 }
      );
      XLSX.utils.book_append_sheet(workbook, worksheet, `Schedule ${i+1}`);
    }

    const rows = presentations.map(elt => [elt.presIdentity, students.find( (s) => s.user.id === elt.student.id).matricNum, elt.student.name, elt.title, elt.supervisor.name, elt.examinerOne.name, elt.examinerTwo.name, elt.venue, elt.timeSlot, elt.date, elt.day]);      
    const data = [];
    data.push(HEADERS);
    rows.forEach((row) => {
      data.push(row);
    ...
    XLSX.utils.book_append_sheet(workbook, newWorksheet, "Presentation Details");

    XLSX.writeFile(workbook, `CSMInD ${selectedCsmind.startDate}.xlsx`);
  }

  const onEdit = () => {
    if(selectedCsmind && selectedCsmind.schedule !== null)
      history.push(`/editschedule/${selectedCsmind.id}`);
  };
